...
import { Modal, Box, Typography, Button, Tab, IconButton, Switch, FormGroup, FormControlLabel, LinearProgress, CircularProgress } from '@mui/material';
    ...
    return (
      <>
        ...
        <TabContext value={tabValue}>
          <Box sx={{ borderBottom: 1, borderColor: 'divider' }}><TabList onChange={handleTabChange} variant="scrollable" scrollButtons="auto"><Tab label="The Plan" value="1" /><Tab label="Challenges" value="2" /><Tab label="Updates" value="3" /><Tab label="Supporters" value="4" />{isCreator && <Tab label="Settings" value="5" />}</TabList></Box>
          <Box sx={{ flexGrow: 1, overflowY: 'auto', py: 2 }}>
            <TabPanel value="1" sx={{p: 0}}><ul style={{ margin: 0, paddingLeft: '20px' }}>{data.plan?.map((item, index) => <li key={index} style={{ textDecoration: item.completed ? 'line-through' : 'none', marginBottom: '8px' }}>{item.text}</li>)}</ul></TabPanel>
            <TabPanel value="2" sx={{p: 0}}>{data.challenges?.map((challenge, index) => <Box key={index} sx={{ mb: 3 }}><Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><Typography variant="h6">{challenge.title}</Typography><Typography variant="caption">{challenge.unit === '$' ? '$' : ''}{challenge.current.toLocaleString()}/{challenge.goal.toLocaleString()} {challenge.unit !== '$' ? challenge.unit : ''}</Typography></Box><Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>{challenge.description}</Typography><LinearProgress variant="determinate" value={challenge.progress * 100} sx={{ height: '8px', borderRadius: '4px' }} /></Box>)}</TabPanel>
            <TabPanel value="3" sx={{p: 0}}>{data.updates?.map((update, index) => <Typography key={index} paragraph>{update}</Typography>)}</TabPanel>
            <TabPanel value="4" sx={{p: 0}}><Typography>A grid of supporter profiles will be shown here.</Typography></TabPanel>
            {isCreator && <TabPanel value="5" sx={{p: 0}}><Typography variant="h6" sx={{mb: 1}}>Campaign Settings</Typography><FormGroup><FormControlLabel control={<Switch color="primary" checked={data.links?.room?.enabled} onChange={() => handleLinkToggle('room')} />} label="Enable Room Link" /><FormControlLabel control={<Switch color="primary" checked={data.links?.messages?.enabled} onChange={() => handleLinkToggle('messages')} />} label="Enable Messages Link" /></FormGroup><Typography variant="caption" color="text.secondary" sx={{mt: 1, display: 'block'}}>Disabling a link will prevent supporters from accessing it directly from this hub.</Typography></TabPanel>}
          </Box>
        </TabContext>
      </>
    );
