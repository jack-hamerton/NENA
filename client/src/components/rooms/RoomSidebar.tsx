      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {activeTab === 'chat' && (
          <div className="flex-1 flex flex-col overflow-hidden">
                ...
                messages.map((msg) => (
                  <div key={msg.id} className="flex gap-3 group">
                    <Avatar className="h-8 w-8 flex-shrink-0">
                      ...
                      <AvatarFallback fallback={msg.username} className="text-[10px] bg-primary/10" />
                    </Avatar>
                    <div className="min-w-0 space-y-1">
                      ...
                      <div className="bg-muted/30 rounded-2xl p-3 text-xs leading-relaxed border border-border/20">
                        {msg.content}
                      </div>
                    </div>
                  </div>
            ...
            {/* Chat Input with Simple Popover AI Assist */}
            <div className="p-4 border-t border-border/50 bg-background/50">
              <div className="flex items-center gap-2 bg-muted/50 rounded-2xl px-3 py-1 border border-border/30 relative">
                <input
                  type="text"
                  value={input}
                ...
                {/* Manual AI Assist Popover */}
                <div className="relative">
                  <Button 
                    variant="ghost" 
                    size="icon" 
                    className={cn("h-7 w-7 rounded-full transition-colors", isAiMenuOpen ? "bg-primary/20 text-primary" : "hover:bg-primary/10")}
                  ...
                  {isAiMenuOpen && (
                    <div className="absolute bottom-full right-0 mb-2 w-40 bg-card border border-border/50 rounded-xl shadow-2xl p-1 z-50 animate-in fade-in slide-in-from-bottom-2 duration-200">
        ...
        {activeTab === 'polls' && (
          <div className="flex-1 p-4 overflow-y-auto space-y-4">
            <div className="bg-primary/5 rounded-xl border border-primary/20 p-4 space-y-3">
               ...
               <p className="text-xs font-medium">Which design style do you prefer for our next drop?</p>
               <div className="space-y-2">
                  <Button variant="outline" className="w-full justify-between text-[11px] h-9 hover:bg-primary/10">
               ...
               <p className="text-[10px] text-muted-foreground text-center">126 people voted</p>
            </div>
            
            <Button variant="outline" className="w-full border-dashed gap-2 text-[10px] h-10">
        ...
        {activeTab === 'users' && (
          <div className="flex-1 p-4 overflow-y-auto space-y-4">
             <div className="flex items-center justify-between">
                <h4 className="text-xs font-bold text-muted-foreground uppercase tracking-widest">In the room</h4>
                <span className="text-[10px] bg-muted px-2 py-0.5 rounded-full">{roomParticipants.length}</span>
             </div>
                  ...
                  <div key={p.id} className="flex items-center justify-between group">
                    <div className="flex items-center gap-3">
                       <Avatar className="h-8 w-8">
                         ...
                         <AvatarFallback fallback={p.username} className="text-[10px]" />
                       </Avatar>
                    ...
                    {p.role === "host" && <Badge variant="secondary" className="text-[8px] h-4">Host</Badge>}
                  </div>
                ))}
             </div>
          </div>
