...
import { ChevronLeft, Share2, Heart, MessageSquare, Info } from "lucide-react";
  ...
  if (!podcast || !selectedEpisode) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 bg-background">
        ...
        <Button onClick={() => router.push("/podcast")}>Go Back</Button>
      </div>
    );
  }
      ...
      <div className="max-w-4xl mx-auto w-full p-6 md:p-10 space-y-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <h3 className="font-bold text-sm uppercase tracking-wider text-muted-foreground">More Episodes</h3>
                ...
                {podcast.episodes.map((ep) => (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedEpisode.id === ep.id ? "bg-primary/10 ring-1 ring-primary/20" : "hover:bg-accent"
                    )}
                  >
                    <div className="h-10 w-10 flex-shrink-0 bg-muted rounded flex items-center justify-center">
                      <Play className={cn("h-4 w-4", selectedEpisode.id === ep.id ? "text-primary fill-current" : "text-muted-foreground")} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className={cn("text-xs font-bold truncate", selectedEpisode.id === ep.id ? "text-primary" : "text-foreground")}>
                        {ep.title}
                      </p>
                      <p className="text-[10px] text-muted-foreground">{ep.releaseDate}</p>
                    </div>
                  </button>
                ))}
              </div>
